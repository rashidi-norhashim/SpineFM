...
def get_data_loader(dataset,data_path,mode='Training',b=4):
    if dataset == 'NHANESII_Vertebrae':
    ...
    elif dataset == 'PointPredictorV2':
        dataset = PointPredictorV2(data_path=data_path,transforms=None,mode=mode)
        data_loader = DataLoader(dataset,batch_size=b,shuffle=True,num_workers=4, collate_fn=collate_fn)
        return data_loader
        ...
        dataset = CSXA(data_path=data_path,transforms=transform,mode=mode)
        data_loader = DataLoader(dataset,batch_size=b,shuffle=True,num_workers=4, collate_fn=collate_fn)
        return data_loader
...
class PointPredictorV2(torch.utils.data.Dataset):
    def __init__(self, data_path, transforms=None,mode='Training'):
        self.data_path = data_path
        self.transforms = transforms
        self.mode = mode
        ...
        self.name_list = [filename.split('.')[0] for filename in os.listdir(os.path.join(data_path,'imgs')) if filename[0:6] in id_list]
    ...
    def __getitem__(self, idx):
        ...
        img_path = os.path.join(self.data_path, 'imgs', id+'.jpg')
        img = Image.open(img_path).convert('RGB')
        img = torch.Tensor(np.array(img)).to(dtype=torch.float32).permute(2, 0, 1)
...
class CSXA(torch.utils.data.Dataset):
    def __init__(self, data_path, transforms=None,mode='Training'):
        self.data_path = data_path
        self.transforms = transforms
